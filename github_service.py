...
class GitHubService:
    ...
    def get_json(
        ...
        if r.status_code == 404:
            if default is not None:
                ...
                obj, sha = self.get_json(path)
                return obj, sha
            return default, None

        if r.status_code in (401, 403):
            raise RuntimeError(
                f"Acesso negado ao {path}. Verifique token e permissões. ({r.status_code})"
            )
    ...
    def put_json(
        ...
        if r.status_code == 409:
            ...
            raise RuntimeError(
                f"Conflito ao salvar {path}: {r2.status_code} {r2.text}"
            )

        if r.status_code in (401, 403):
            raise RuntimeError(
                f"Acesso negado ao salvar {path}. Verifique token e permissões. ({r.status_code})"
            )
