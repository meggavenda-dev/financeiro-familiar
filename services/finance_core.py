...
def gerar_parcelas(item_base: dict, qtd_parcelas: int, intervalo_meses: int = 1) -> list:
    ...
    base = (total / Decimal(qtd_parcelas)).quantize(Decimal("0.01"))
    valores = [base] * qtd_parcelas
    ajuste = (total - sum(valores)).quantize(Decimal("0.01"))
    valores[-1] = (valores[-1] + ajuste).quantize(Decimal("0.01"))
    ...
    for i in range(qtd_parcelas):
        ...
        p["id"] = novo_id("tx")
        p["valor"] = float(valores[i])  # persistir como float
...
def normalizar_tx(d):
    ...
    Não persiste 'status' (status sempre derivado no UI/páginas).
    """
