...
from services.finance_core import novo_id  # para gerar IDs, se faltarem
...
# --------------------------------------------------
# Sanitização defensiva (garante estrutura e IDs)
# --------------------------------------------------
...
for o in orcamentos_raw:
    ...
    categoria_id = o.get("categoria_id")  # pode ser None/ID inválido; manter como está para não perder vínculo
    try:
    ...
    # Se geramos um novo id, marcar mudança
    if o.get("id") is None:
        changed = True

# Persiste sanitização (se necessário)
if changed:
...
# --------------------------------------------------
# Listagem (única visualização organizada)
# --------------------------------------------------
...
if not orcamentos:
    ...
    if not filtered:
        ...
        # Edição por linha (sem KeyError, com índices seguros)
        for o in filtered:
            ...
            with c2:
                novo_lim = st.number_input(
                    ...
                    min_value=0.01,
                    step=0.01,
                    value=float(o.get("limite_mensal", 0.0)),
                    key=f"orc-lim-{oid}"
                )
            with c3:
                novo_ativo = st.checkbox(
                    "Ativo",
                    value=bool(o.get("ativo", True)),
                    key=f"orc-ativo-{oid}"
                )
            with c4:
